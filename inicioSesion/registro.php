...
if ($_SERVER["REQUEST_METHOD"] == "POST") { //este codigo sirve para realizar un if en el que se verifica si el metodo de envio es post
    $usuario = mysqli_real_escape_string($conexion, $_POST['usuario']);// manda a llamar al post de usuario
    $clave = mysqli_real_escape_string($conexion, $_POST['clave']);//manda a llamar al post de clave
    $password_hash = password_hash($clave, PASSWORD_BCRYPT);//antes de enviarla a la base de datos la encripta
